...
使用 [Page.waitForRequest()](/api/class-page.mdx#page-wait-for-request) 等待指定 URL 的請求：

```java
// 回調 lambda 定義預期觸發請求的程式碼範圍。
