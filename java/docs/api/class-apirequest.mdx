...
- `options` `ApiRequest.NewContextOptions` *(optional)*
  ...
  - `setStorageState` [String] *(optional)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="api-request-new-context-option-storage-state"/><a href="#api-request-new-context-option-storage-state" class="list-anchor">#</a>
    ...
    使用給定的儲存狀態填入情境。此選項可用於透過 [BrowserContext.storageState()](/api/class-browsercontext.mdx#browser-context-storage-state) 或 [APIRequestContext.storageState()](/api/class-apirequestcontext.mdx#api-request-context-storage-state) 取得的登入資訊來初始化情境。可以是儲存檔案的路徑，或是 [BrowserContext.storageState()](/api/class-browsercontext.mdx#browser-context-storage-state) 或 [APIRequestContext.storageState()](/api/class-apirequestcontext.mdx#api-request-context-storage-state) 方法回傳的值。
  - `setStorageStatePath` [Path] *(optional)* <font size="2">Added in: v1.18</font><a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="api-request-new-context-option-storage-state-path"/><a href="#api-request-new-context-option-storage-state-path" class="list-anchor">#</a>
    ...
    使用給定的儲存狀態填入情境。此選項可用於透過 [BrowserContext.storageState()](/api/class-browsercontext.mdx#browser-context-storage-state) 取得的登入資訊來初始化情境。儲存狀態檔案的路徑。
