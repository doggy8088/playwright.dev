...
- `options` `ApiRequest.NewContextOptions` *(optional)*
  - `setBaseURL` [String] *(optional)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="api-request-new-context-option-base-url"/><a href="#api-request-new-context-option-base-url" class="list-anchor">#</a>
    ...
    Methods like [APIRequestContext.get()](/api/class-apirequestcontext.mdx#api-request-context-get) 會透過使用 [`URL()`](https://developer.mozilla.org/en-US/docs/Web/API/URL/URL) 建構函式來建立對應的 URL，將基礎 URL 納入考慮。範例：
  ...
  - `setStorageState` [String] *(optional)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="api-request-new-context-option-storage-state"/><a href="#api-request-new-context-option-storage-state" class="list-anchor">#</a>
    ...
    使用給定的儲存狀態填入情境。此選項可用於使用透過 [BrowserContext.storageState()](/api/class-browsercontext.mdx#browser-context-storage-state) 或 [APIRequestContext.storageState()](/api/class-apirequestcontext.mdx#api-request-context-storage-state) 取得的已登入資訊來初始化情境。可以是儲存檔案的路徑，或是 [BrowserContext.storageState()](/api/class-browsercontext.mdx#browser-context-storage-state) 或 [APIRequestContext.storageState()](/api/class-apirequestcontext.mdx#api-request-context-storage-state) 方法傳回的值。
  - `setStorageStatePath` [Path] *(optional)* <font size="2">Added in: v1.18</font><a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="api-request-new-context-option-storage-state-path"/><a href="#api-request-new-context-option-storage-state-path" class="list-anchor">#</a>
    ...
    使用給定的儲存狀態填入情境。此選項可用於使用透過 [BrowserContext.storageState()](/api/class-browsercontext.mdx#browser-context-storage-state) 取得的已登入資訊來初始化情境。已儲存儲存狀態的檔案路徑。
