...
### 其他
- [APIRequest.newContext()](/api/class-apirequest.mdx#api-request-new-context) 中的新選項 [setMaxRedirects](/api/class-apirequest.mdx#api-request-new-context-option-max-redirects) 用於控制重定向的最大次數。
...
### 重點
* [BrowserContext.storageState()](/api/class-browsercontext.mdx#browser-context-storage-state) 的新選項 [setIndexedDB](/api/class-browsercontext.mdx#browser-context-storage-state-option-indexed-db) 允許儲存和還原 IndexedDB 內容。當您的應用程式使用 [IndexedDB API](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API) 儲存驗證權杖時很有用，例如 Firebase Authentication。
...
* 新選項 [setVisible](/api/class-locator.mdx#locator-filter-option-visible) for [Locator.filter()](/api/class-locator.mdx#locator-filter) allows matching only visible elements.
...
* 新選項 [setFailOnStatusCode](/api/class-apirequest.mdx#api-request-new-context-option-fail-on-status-code) makes all fetch requests made through the [APIRequestContext] throw on response codes other than 2xx and 3xx.
...
- TLS client certificates 現在可以 be passed from memory by passing [setClientCertificates.setCert](/api/class-browser.mdx#browser-new-context-option-client-certificates) and [setClientCertificates.setKey](/api/class-browser.mdx#browser-new-context-option-client-certificates) as byte arrays instead of file paths.
...
**Locator handler**
- After executing the handler added with [Page.addLocatorHandler()](/api/class-page.mdx#page-add-locator-handler), Playwright 現在會 wait until the overlay that triggered the handler is not visible anymore. You can opt-out of this behavior with the new `setNoWaitAfter` option.
...
- 新方法 [FormData.append()](/api/class-formdata.mdx#form-data-append) 允許 specify repeating fields with the same name in [`setMultipart`](./api/class-requestoptions#request-options-set-multipart) option in `RequestOptions`:
...
- 新選項s [setStyle](/api/class-page.mdx#page-screenshot-option-style) in [Page.screenshot()](/api/class-page.mdx#page-screenshot) and [setStyle](/api/class-locator.mdx#locator-screenshot-option-style) in [Locator.screenshot()](/api/class-locator.mdx#locator-screenshot) to add custom CSS to the page before taking a screenshot.
...
- 新選項 [setHasNot](/api/class-locator.mdx#locator-filter-option-has-not) in [Locator.filter()](/api/class-locator.mdx#locator-filter)
- 新選項 [setHasNotText](/api/class-locator.mdx#locator-filter-option-has-not-text) in [Locator.filter()](/api/class-locator.mdx#locator-filter)
- [assertThat(locator).isAttached()](/api/class-locatorassertions.mdx#locator-assertions-to-be-attached)
- 新選項 [setTimeout](/api/class-route.mdx#route-fetch-option-timeout) in [Route.fetch()](/api/class-route.mdx#route-fetch)
...
- 新選項 [setName](/api/class-tracing.mdx#tracing-start-chunk-option-name) in method [Tracing.startChunk()](/api/class-tracing.mdx#tracing-start-chunk).
...
- 新選項 [setMaxRedirects](/api/class-route.mdx#route-fetch-option-max-redirects) for method [Route.fetch()](/api/class-route.mdx#route-fetch).
...
- Playwright for Java now supports **Ubuntu 20.04 ARM64** and **Apple M1**. You 現在可以 run Playwright for Java tests on Apple M1, inside Docker on Apple M1, and on Raspberry Pi.
