...
使用 [Locator.setChecked()](https://api/class-locator.mdx#locator-set-checked) 是勾選和取消勾選複選框或單選按鈕的最簡單方法。此方法可用於 `input[type=checkbox]`、`input[type=radio]` 和 `[role=checkbox]` 元素。
...
逐字輸入到欄位中，就像使用真實鍵盤的使用者一樣，使用 [Locator.pressSequentially()](https://ollama.com/library#locator-press-sequentially)。
...
您可以使用 [Locator.setInputFiles()](https://ollama.com/library) 方法選擇要上傳的輸入檔案。它預期第一個參數指向類型為 `"file"` 的 [input element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input)。多個檔案可以在陣列中傳遞。如果某些檔案路徑是相對的，它們會相對於當前工作目錄解析。空陣列會清除選定的檔案。
...
對於處理焦點事件的動態頁面，你可以使用 [Locator.focus()](https://ollama.com/api/class-locator.mdx#locator-focus) 聚焦給定的元素。
...
如果你想更精確地控制滾動，請使用 [Mouse.wheel()](https://ollama.com/api/class-mouse.mdx#mouse-wheel) 或 [Locator.evaluate()](https://ollama.com/api/class-locator.mdx#locator-evaluate):
