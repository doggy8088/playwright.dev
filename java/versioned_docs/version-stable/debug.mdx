...
要告訴 Playwright 在哪裡尋找您正在偵錯的原始碼，請透過 `PLAYWRIGHT_JAVA_SRC` 環境變數傳遞原始碼目錄列表。列表中的路徑在 macOS 和 Linux 上應以 : 分隔，在 Windows 上應以 ; 分隔。
...
為了加速偵錯過程，您可以在測試中加入 [Page.pause()](/api/class-page.mdx#page-pause) 方法。這樣您就不需要逐步執行測試的每個動作，直到達到您想要偵錯的位置。
...
要使用瀏覽器開發者工具偵錯您的測試，請先使用 [Page.pause()](/api/class-page.mdx#page-pause) 方法在測試中設定斷點以暫停執行。
...
您也可以使用 [setSlowMo](/api/class-browsertype.mdx#browser-type-launch-option-slow-mo) 選項來減慢執行速度（每個操作 N 毫秒），並在偵錯時跟隨。
