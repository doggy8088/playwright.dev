...
等待 [Response]s 與 [Page.waitForResponse()](https://api/class-page.mdx#page-wait-for-response)
...
在整個瀏覽器上下文中設定路由，使用 [BrowserContext.route()](https://ollama.com/api/class-browsercontext.mdx#browser-context-route) 或頁面使用 [Page.route()](https://ollama.com/api/class-page.mdx#page-route)。它將適用於彈出視窗和打開的連結。
...
1. 如果你正在使用 Playwright 的原生 [BrowserContext.route()](/api/class-browsercontext.mdx#browser-context-route) 和 [Page.route()](/api/class-page.mdx#page-route)，並且似乎缺少網路事件，請通過將 `Browser.newContext.serviceWorkers` 設置為 `'block'` 來禁用 Service Workers。
