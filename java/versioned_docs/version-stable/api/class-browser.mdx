...
A Browser is created via [BrowserType.launch()](/api/class-browsertype.mdx#browser-type-launch)建立的。以下是使用 [Browser] 建立 [Page] 的範例：
...
回傳所有開啟的瀏覽器情境陣列。 在新建立的瀏覽器中，這將回傳零個瀏覽器情境。
...
CDP Session 僅在基於 Chromium 的瀏覽器上支援。
:::
...
建立新的瀏覽器情境。 它不會與其他瀏覽器情境共享 cookies/快取。
