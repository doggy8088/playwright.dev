...
每個頁面下載的附件都會觸發 [Page.onDownload(handler)](/api/class-page.mdx#page-event-download) 事件。所有這些附件都會下載到一個臨時資料夾中。你可以使用事件中的 [Download] 物件獲取下載的 URL、檔案名稱和有效負載流。

您可以使用 `downloadsPath` 選項來指定下載的檔案儲存位置 [BrowserType.launch()](/api/class-browsertype.mdx#browser-type-launch)。
