...
- `article:has-text("Playwright")` - `:has-text()` 匹配包含指定文字的任何元素，可能在子元素或後代元素內部。匹配不區分大小寫，會修剪空白並搜索子字串。

  例如，`article:has-text("Playwright")` 匹配 `<article><div>Playwright</div></article>`。

  注意，`:has-text()` 應與其他 CSS 指定符一起使用，否則它將匹配包含指定文字的所有元素，包括 `<body>`。
...
- `#nav-bar :text("Home")` - `:text()` 偽類匹配包含指定文字的最小元素。匹配不區分大小寫，會修剪空白並搜索子字串。

  例如，這將在 `#nav-bar` 元素內部找到包含文字 "Home" 的元素：

  ```java
  page.locator("#nav-bar :text('Home')").click();
  ```

- `#nav-bar :text-is("Home")` - `:text-is()` 偽類匹配具有精確文字的最小元素。精確匹配區分大小寫，會修剪空白並搜索完整字串。
...
請注意，結果匹配項是按與錨點元素的距離排序的，因此您可以使用 [Locator.first()](https://ollama.com/api/class-locator.mdx#locator-first) 來選擇最近的一個。這只有在您有類似元素列表時才有用，因為最近的顯然是正確的。然而，在其他情況下使用 [Locator.first()](https://ollama.com/api/class-locator.mdx#locator-first) 很可能不會如預期般工作——它不會定位到您正在搜尋的元素，而是一些其他碰巧是最近的元素，如隨機的空 `<div>`，或是已滾動出視野且當前不可見的元素。
...
如果你想針對文本為 "Hello" 的標籤的父 `<li>`，使用 [Locator.filter()](/api/class-locator.mdx#locator-filter) 最合適：
...
或者，如果您找不到適合的父元素定位器，請使用 `xpath=..`。請注意，此方法不太可靠，因為對 DOM 結構的任何更改都會破壞您的測試。盡可能使用 [Locator.filter()](/api/class-locator.mdx#locator-filter)。
...
例如，考慮以下 DOM 結構: `<label for="password">Password:</label><input id="password" type="password">`。你可以使用 [Page.getByText()](https://ollama.com/library) 透過其 "Password" 文字來定位標籤。然而，以下操作將在輸入框上執行，而不是標籤上:
...
然而，其他方法將針對標籤本身，例如 [assertThat(locator).hasText](/api/class-locatorassertions.mdx#locator-assertions-to-have-text) 會斷言標籤的文本內容，而不是輸入欄位。
...
- `text="Log in"` - 文字內容可以用單引號或雙引號轉義，以搜索修剪空白後具有精確內容的文字節點。
  
  例如，`text="Log"` 不匹配 `<button>Log in</button>`，因為 `<button>` 包含一個單一的文字節點 `"Log in"`，這不等於 `"Log"`。然而，`text="Log"` 匹配 `<button> Log <span>in</span></button>`，因為 `<button>` 包含一個文字節點 `" Log "`。這種精確模式意味著區分大小寫匹配，所以 `text="Download"` 不會匹配 `<button>download</button>`。
