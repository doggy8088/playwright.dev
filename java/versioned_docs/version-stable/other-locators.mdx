...
請注意，結果匹配項是按與錨點元素的距離排序的，因此您可以使用 [Locator.first()](/api/class-locator.mdx#locator-first) 來選擇最近的一個。這只有在您有類似元素列表時才有用，因為最近的顯然是正確的。然而，在其他情況下使用 [Locator.first()](/api/class-locator.mdx#locator-first) 很可能不會如預期般工作——它不會定位到您正在搜尋的元素，而是一些其他碰巧是最近的元素，如隨機的空 `<div>`，或是已滾動出視野且當前不可見的元素。
...
例如，考慮以下 DOM 結構: `<label for="password">Password:</label><input id="password" type="password">`。你可以使用 [Page.getByText()](/api/class-page.mdx#page-get-by-text) 透過其 "Password" 文字來定位標籤。然而，以下操作將在輸入框上執行，而不是標籤上:
