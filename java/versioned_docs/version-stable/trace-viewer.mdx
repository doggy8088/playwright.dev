...
當使用 [setScreenshots](/api/class-tracing.mdx#tracing-start-option-screenshots) 選項開啟追蹤時（預設），每個追蹤會記錄螢幕錄影並將其呈現為膠片條。您可以懸停在膠片條上查看每個動作和狀態的放大影像，這有助於您輕鬆找到要檢查的動作。
...
當使用 [setSnapshots](/api/class-tracing.mdx#tracing-start-option-snapshots) 選項開啟追蹤時（預設），Playwright 會為每個動作捕獲一組完整的 DOM 快照。根據動作的類型，它將捕獲：
