...
- 現在在附件窗格中內嵌顯示文字附件的內容。
- 新增設定以顯示/隱藏路由操作，如 [Route.resume()](/api/class-route.mdx#route-continue)。
- 請求方法和狀態顯示在網路詳細資訊標籤中。
- 新按鈕可將來源檔案位置複製到剪貼簿。
...
- 新方法 [FormData.append()](/api/class-formdata.mdx#form-data-append) 允許在 `RequestOptions` 的 [`setMultipart`](./api/class-requestoptions#request-options-set-multipart) 選項中指定具有相同名稱的重複字段:
...
新增新的 [`@UsePlaywright`](./junit.mdx) 註釋到您的測試類別，以便在測試方法中開始使用 Playwright fixtures 來測試 [Page]、[BrowserContext]、[Browser]、[APIRequestContext] 和 [Playwright]。
...
新方法 [Page.addLocatorHandler()](/api/class-page.mdx#page-add-locator-handler) 註冊了一個回呼，當指定元素變得可見時將會被呼叫，並且可能會阻擋 Playwright 操作。回呼可以去除覆蓋層。這裡有一個範例，當 cookie 對話框出現時將其關閉。
...
* 新選項 `maskColor` 用於方法 [Page.screenshot()](/api/class-page.mdx#page-screenshot) 和 [Locator.screenshot()](/api/class-locator.mdx#locator-screenshot) 來更改預設遮罩顏色。
...
- 新方法 [Locator.all()](/api/class-locator.mdx#locator-all) 來迭代所有匹配的元素:
...
- [Page.GetByRole()](/api/class-page.mdx#page-get-by-role) 根據 [ARIA 角色](https://www.w3.org/TR/wai-aria-1.2/#roles)、[ARIA 屬性](https://www.w3.org/TR/wai-aria-1.2/#aria-attributes) 和 [可訪問名稱](https://w3c.github.io/accname/#dfn-accessible-name) 進行定位。
- [Page.GetByLabel()](/api/class-page.mdx#page-get-by-label) 根據關聯標籤的文本定位表單控件。
- [Page.GetByTestId()](/api/class-page.mdx#page-get-by-test-id) 根據其 `data-testid` 屬性（可以配置其他屬性）定位元素。
- [Page.GetByPlaceholder()](/api/class-page.mdx#page-get-by-placeholder) 根據佔位符定位輸入框。
- [Page.GetByAltText()](/api/class-page.mdx#page-get-by-alt-text) 根據替代文本定位元素，通常是圖像。
...
- [assertThat(locator).hasText()](/api/class-locatorassertions.mdx#locator-assertions-to-have-text) 現在穿透開放的 shadow roots。
- 新選項 `editable` 用於 [assertThat(locator).isEditable()](/api/class-locatorassertions.mdx#locator-assertions-to-be-editable)。
- 新選項 `visible` 用於 [assertThat(locator).isVisible()](/api/class-locatorassertions.mdx#locator-assertions-to-be-visible)。

### 其他亮點

- 新選項 `setMaxRedirects` 用於 [APIRequestContext.get()](/api/class-apirequestcontext.mdx#api-request-context-get) 和其他用來限制重定向次數。
...
- 新的 `caret` 選項在 [Page.screenshot()](./api/class-page.mdx#page-screenshot) 中控制文本插入符號。預設為 `"hide"`。

### 行為變更

- Playwright 現在支援大型檔案上傳 (數百 MB) 透過 [Locator.setInputFiles()](/api/class-locator.mdx#locator-set-input-files) API。
...
要代表 Playwright 的 Page 進行請求，使用 **new [Page.request()](/api/class-page.mdx#page-request) API**:
...
- [BrowserType.launch()](/api/class-browsertype.mdx#browser-type-launch) 現在接受新的 'channel' 選項。更多資訊請參閱[我們的文件](./browsers)。

## Version 1.9

- [Playwright Inspector](./debug.mdx) 是一個**新的 GUI 工具**，用於編寫和偵錯你的測試。
  - **逐行偵錯**你的 Playwright 腳本，具有播放、暫停和逐步執行功能。
  - 通過**錄製用戶操作**來編寫新腳本。
  - 通過懸停在元素上來**生成元素選擇器**。
  - 設置 `PWDEBUG=1` 環境變量以啟動 Inspector
- 在有頭模式下使用 [Page.pause()](./api/class-page.mdx#page-pause) **暫停腳本執行**。暫停頁面會啟動 [Playwright Inspector](./debug.mdx) 進行偵錯。
...
- [BrowserContext.storageState()](/api/class-browsercontext.mdx#browser-context-storage-state) 來獲取當前狀態以供日後重用。
