...
Playwright 內建多個定位器。為了讓測試更具韌性，我們建議優先考慮面向使用者的屬性和明確的合約，例如 [Page.getByRole()](https://ollama.com/library)。
...
請注意，所有建立定位器的方法，例如 [Page.getByLabel()](https://ollama.com/api/class-page.mdx#page-get-by-label)，也可在 [Locator] 和 [FrameLocator] 類別上使用，因此您可以鏈接它們並逐步縮小您的定位器範圍。
...
[Page.getByRole()](https://ollama.com/library#page-get-by-role) 定位器反映了使用者和輔助技術如何感知頁面，例如某個元素是按鈕還是複選框。當按角色定位時，你通常應該傳遞可訪問的名稱，以便定位器能夠精確定位到具體的元素。
...
找到包含特定文字的元素。使用 [Page.getByText()](https://ollama.com/library#page-get-by-text) 時，你可以透過子字串、精確字串或正則表達式來匹配。
...
所有圖片應該有一個 `alt` 屬性來描述圖片。你可以使用 [Page.getByAltText](https://ollama.com/library) 根據文字替代項來定位圖片。
...
測試 id 是最具彈性的測試方式，因為即使您的文字或屬性的角色發生變化，測試仍然會通過。QA 和開發人員應該定義明確的測試 id 並使用 [Page.getByTestId()](https://ollama.com/library) 進行查詢。然而，通過測試 id 進行測試並不是面向使用者的。如果角色或文字值對您很重要，請考慮使用面向使用者的定位器，例如 [role](#locate-by-role) 和 [text locators](#locate-by-text)。
...
默認情況下，[Page.getByTestId()](/api/class-page.mdx#page-get-by-test-id) 將根據 `data-testid` 屬性定位元素，但您可以在測試配置中進行配置，或通過呼叫 [Selectors.setTestIdAttribute()](/api/class-selectors.mdx#selectors-set-test-id-attribute) 來配置。
...
定位器可以通過 [Locator.filter()](https://ollama.com/api/class-locator.mdx#locator-filter) 方法根據文本過濾。它將在元素內部的某個地方（可能在後代元素中）搜索特定字符串，不區分大小寫。你也可以傳遞正則表達式。
...
方法 [Locator.and()](https://api/class-locator.mdx#locator-and) 通過匹配額外的定位器來縮小現有定位器的範圍。例如，你可以結合 [Page.getByRole()](https://api/class-page.mdx#page-get-by-role) 和 [Page.getByTitle()](https://api/class-page.mdx#page-get-by-title) 來同時按角色和標題進行匹配。
...
如果你想要定位其中一個或多個元素，並且你不知道會是哪一個，使用 [Locator.or()](https://api/class-locator.mdx#locator-or) 來建立一個匹配所有替代方案的定位器。
...
使用 [Locator.filter()](https://ollama.com/api/class-locator.mdx#locator-filter) 來定位清單中的特定項目。
...
使用 [Page.getByTestId()](https://ollama.com/api/class-page.mdx#page-get-by-test-id) 方法來定位列表中的元素。如果你還沒有測試 ID，你可能需要修改 HTML 並添加測試 ID。
...
如果你有一個相同元素的列表，且區分它們的唯一方法是順序，你可以從列表中選擇一個特定元素，使用 [Locator.first()](https://ollama.com/api/class-locator.mdx#locator-first)、[Locator.last()](https://ollama.com/api/class-locator.mdx#locator-last) 或 [Locator.nth()](https://ollama.com/api/class-locator.mdx#locator-nth)。
...
當你有具有各種相似性的元素時，你可以使用 [Locator.filter()](https://ollama.com/api/class-locator.mdx#locator-filter) 方法來選擇正確的元素。你也可以串聯多個篩選器來縮小選擇範圍。
...
您可以通過告訴 Playwright 當多個元素匹配時使用哪個元素來明確選擇退出嚴格性檢查，通過 [Locator.first()](https://api/class-locator.mdx#locator-first)、[Locator.last()](https://api/class-locator.mdx#locator-last) 和 [Locator.nth()](https://api/class-locator.mdx#locator-nth)。這些方法**不推薦**，因為當您的頁面更改時，Playwright 可能會點擊您未打算點擊的元素。相反，請遵循上述最佳實踐來建立唯一識別目標元素的定位器。
