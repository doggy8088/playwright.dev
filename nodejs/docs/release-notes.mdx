...
您現在可以合併來自多個檔案或模組的自訂 expect 匹配器：
...
* UI 模式和追蹤檢視器會標記由 [page.route()](/api/class-page.mdx#page-route) 和 [browserContext.route()](/api/class-browsercontext.mdx#browser-context-route) 處理器處理的網路請求，以及透過 [API testing](./api-testing) 發出的請求：
...
### ⚠️ Breaking changes
* `playwright-core` 執行檔已從 `playwright` 重新命名為 `playwright-core`。因此，如果您使用 `playwright-core` CLI，請務必更新名稱：
...
### 重點
* UI 模式現在顯示步驟、佈置和附件：![UI 模式附件](https://github.com/microsoft/playwright/assets/746130/1d280419-d79a-4a56-b2dc-54d631281d56)
* 新屬性 [testProject.teardown](/api/class-testproject.mdx#test-project-teardown) 用於指定在此專案和所有相依專案完成後需要執行的專案。清理對於清理此專案取得的任何資源很有用。
  
  常見的模式是 `setup` 相依性搭配對應的 `teardown`：
...
- 新方法 [locator.all()](/api/class-locator.mdx#locator-all) 用於遍歷所有符合的元素：
...
### 測試執行器
* 使用 [test.describe.configure()](/api/class-test.mdx#test-describe-configure) 為檔案或測試配置重試次數和測試逾時。
  
  ```js
  // 檔案中的每個測試都會重試兩次，並具有 20 秒的逾時。
...
### 公告
* 🎁 我們現在提供Ubuntu 22.04 Jammy Jellyfish docker 映像：`mcr.microsoft.com/playwright:v1.34.0-jammy`。
...
- 新的 `maxDiffPixels` 和 `maxDiffPixelRatio` 選項用於使用 `expect().toMatchSnapshot()` 進行細粒度螢幕截圖比較：
  ...
  最便利的方式是在 [testConfig.expect](/api/class-testconfig.mdx#test-config-expect) 中一次指定 `maxDiffPixels` 或 `maxDiffPixelRatio`。
...
- 您現在可以指定**自訂 expect 訊息**作為 `expect` 和 `expect.soft` 函數的第二個參數，例如：
  ...
  ```js
  await expect(page.locator('text=Name'), '應該已登入').toBeVisible();
  ```
...
// fixtures.js
const test = base.extend({
  // 標記為「選項: true」的 fixture 將取得在配置中指定的值，
  // 或回到預設值。
...
- HTML 報告獲得了[新的配置選項](./test-reporters#html-reporter)
...
- [`testOptions.trace`](./api/class-testoptions#test-options-trace) 具有新選項
...
### Arm64 的 Docker 支援Playwright Docker 映像現在發佈用於 Arm64，因此可以在 Apple Silicon 上使用。

在 [Docker 整合](./docker)中閱讀更多。

### 🎭 Playwright Trace Viewer
- web-first assertions inside trace viewer
- 使用 `npx playwright show-trace` 執行追蹤檢視器並將追蹤檔案拖放到追蹤檢視器 PWA
- API 測試與追蹤檢視器整合
...
- [page.setChecked()](/api/class-page.mdx#page-set-checked) 和 [locator.setChecked()](/api/class-locator.mdx#locator-set-checked) 被引入以設定核取方塊的已選取狀態。
- [request.sizes()](/api/class-request.mdx#request-sizes) 回傳給定 HTTP 要求的資源大小資訊。
- [tracing.startChunk()](/api/class-tracing.mdx#tracing-start-chunk) - 開始新的追蹤區塊。
- [tracing.stopChunk()](/api/class-tracing.mdx#tracing-stop-chunk) - 停止新的追蹤區塊。
...
預設情況下，單一檔案中的測試會按順序執行。如果您在單一檔案中有許多獨立測試，您現在可以使用 [test.describe.parallel(title, callback)](./api/class-test#test-describe-parallel) 平行執行它們。
...
[Locator](./api/class-locator) 和 [ElementHandle](./api/class-elementhandle) 之間的差異在於後者指向特定元素，而 [Locator](./api/class-locator) 捕獲如何檢索該元素的邏輯。
...
- [`nth`](./other-locators.mdx#n-th-element-locator) 選擇器引擎等同於 `:nth-match` 偽類別，但可以與其他選擇器引擎結合使用。
- [`visible`](./other-locators.mdx#css-matching-only-visible-elements) 選擇器引擎等同於 `:visible` 偽類別，但可以與其他選擇器引擎結合使用。
...
宣告一組應該始終序列執行的測試。如果其中一個測試失敗，所有後續測試都會被跳過。群組中的所有測試會一起重試。
...
要在測試期間啟動伺服器，請在配置檔案中使用 [`webServer`](./test-webserver) 選項。伺服器會等待指定的 URL 可用後才執行測試，並且該 URL 會在建立情境時作為 [`baseURL`](./api/class-testoptions#test-options-base-url) 傳遞給 Playwright。
...
- **⛺️ 新的 [`baseURL` 佈置](./test-configuration#basic-configuration)** 以支援測試中的相對路徑。

#### Playwright
- **🖖 Programmatic drag-and-drop support** via the [page.dragAndDrop()](/api/class-page.mdx#page-drag-and-drop) API.
- **🔎 Enhanced HAR** 具有請求和回應的主體大小。透過 [browser.newContext()](/api/class-browser.mdx#browser-new-context) 中的 `recordHar` 選項使用。
...
#### 新的 Playwright API
- 新的 `baseURL` 選項在 [browser.newContext()](/api/class-browser.mdx#browser-new-context) 和 [browser.newPage()](/api/class-browser.mdx#browser-new-page) 中
...
使用新的 [browserContext.tracing](/api/class-browsercontext.mdx#browser-context-tracing) API 錄製追蹤：
...
- 使用[新的頻道 API](./browsers) 在 **Google Chrome** 和 **Microsoft Edge** 穩定頻道上執行 Playwright。
...
- [Playwright Inspector](./debug.mdx) 是一個**新的 GUI 工具**，用於編寫和偵錯您的測試。
  - **逐行除錯**您的 Playwright 腳本，包含播放、暫停和逐步執行。
  - 透過**錄製使用者操作**編寫新腳本。
  - 透過滑鼠懸停在元素上為您的腳本**產生元素選擇器**。
  - 設定 `PWDEBUG=1` 環境變數以啟動檢查器
- 在有頭模式中使用 [page.pause()](/api/class-page.mdx#page-pause) **暫停腳本執行**。暫停頁面會啟動 [Playwright Inspector](./debug.mdx) 進行除錯。
...
## 版本 1.8
- 使用 `:left-of()`、`:right-of()`、`:above()` 和 `:below()` 進行[基於佈局的元素選擇](./other-locators.mdx#css-matching-elements-based-on-layout)。
...
- 新方法用於[斷言元素狀態](./actionability#assertions)，如 [page.isEditable()](/api/class-page.mdx#page-is-editable)。
...
- `storageState` 選項在 [browser.newContext()](/api/class-browser.mdx#browser-new-context) 和 [browser.newPage()](/api/class-browser.mdx#browser-new-page) 中用於設定瀏覽器上下文狀態。
