...
此 API 用於 Web API 測試。您可以使用它來觸發 API 端點、配置微服務、準備環境或為您的端對端測試準備服務。

每個 Playwright 瀏覽器情境都有關聯的 [APIRequestContext] 執行個體，該執行個體與瀏覽器情境共享 cookie 儲存，並可透過 [browserContext.request](/api/class-browsercontext.mdx#browser-context-request) 或 [page.request](/api/class-page.mdx#page-request) 存取。也可以透過呼叫 [apiRequest.newContext()](/api/class-apirequest.mdx#api-request-new-context) 手動建立新的 APIRequestContext 執行個體。

**Cookie 管理**

由 [browserContext.request](/api/class-browsercontext.mdx#browser-context-request) 和 [page.request](/api/class-page.mdx#page-request) 回傳的 [APIRequestContext] 與對應的 [BrowserContext] 共享 cookie 儲存。每個 API 請求都會以來自瀏覽器情境的值填入 `Cookie` 標頭。如果 API 回應包含 `Set-Cookie` 標頭，它會自動更新 [BrowserContext] cookies，而從頁面發出的請求會取得它們。這意味著如果您使用此 API 登入，您的端對端測試將會登入，反之亦然。

如果您希望 API 請求不干擾瀏覽器 cookies，您應該透過呼叫 [apiRequest.newContext()](/api/class-apirequest.mdx#api-request-new-context) 建立新的 [APIRequestContext]。這樣的 `APIRequestContext` 物件將擁有自己的隔離 cookie 儲存。
...
傳送 HTTP(S) [DELETE](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/DELETE) 請求並回傳其回應。此方法會從情境填入請求 cookies 並從回應更新情境 cookies。此方法會自動跟隨重新導向。
