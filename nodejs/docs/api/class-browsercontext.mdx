...
BrowserContexts 提供一種操作多個獨立瀏覽器會話的方式。

如果頁面開啟另一個頁面，例如透過 `window.open` 呼叫，彈出視窗將屬於父頁面的瀏覽器情境。

Playwright 允許使用 [browser.newContext()](/api/class-browser.mdx#browser-new-context) 方法建立隔離的非持久性瀏覽器情境。非持久性瀏覽器情境不會將任何瀏覽資料寫入磁碟。
...
Gets the browser instance that owns the context. 回傳值 `null` if the context is created outside of normal browser, e.g. Android or Electron.
...
回傳值 all open pages in the context.
...
回傳值 storage state for this browser context, contains current cookies, local storage snapshot and IndexedDB snapshot.
...
Waits for event to fire and passes its value into the predicate function. 回傳值 when the predicate returns truthy value. Will throw an error if the context closes before the event is fired. 回傳值 the event data value.
