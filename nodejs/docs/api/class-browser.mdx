...
`Browser` 是透過 [browserType.launch()](/api/class-browsertype.mdx#browser-type-launch) 建立的。以下是使用 [Browser] 建立 [Page] 的範例：
...
如果此瀏覽器是使用 [browserType.launch()](/api/class-browsertype.mdx#browser-type-launch) 取得的，則關閉瀏覽器及其所有頁面（如果有開啟的話）。

如果此瀏覽器是連接到的，則清除屬於此瀏覽器的所有已建立的情境並與瀏覽器伺服器斷開連接。

:::note

這類似於強制退出瀏覽器。若要優雅地關閉頁面並確保您接收到頁面關閉事件，請在呼叫 [browser.close()](/api/class-browser.mdx#browser-close) **之前**，對您先前使用 [browser.newContext()](/api/class-browser.mdx#browser-new-context) 明確建立的任何 [BrowserContext] 實例呼叫 [browserContext.close()](/api/class-browsercontext.mdx#browser-context-close)。
:::

[Browser] 物件本身被視為已丟棄，無法再使用。
...
如果直接使用此方法建立 [BrowserContext]，最佳實務是在您的程式碼完成 [BrowserContext] 的使用時，且在呼叫 [browser.close()](/api/class-browser.mdx#browser-close) 之前，透過 [browserContext.close()](/api/class-browsercontext.mdx#browser-context-close) 明確關閉回傳的情境。這將確保 `context` 優雅地關閉，並且任何工件（如 HAR 和影片）都能完全刷新並儲存。
:::
...
在新的瀏覽器情境中建立新頁面。關閉此頁面也會關閉情境。

這是一個便利的 API，僅應用於單頁場景和簡短片段。正式環境程式碼和測試框架應明確建立 [browser.newContext()](/api/class-browser.mdx#browser-new-context)，然後再使用 [browserContext.newPage()](/api/class-browsercontext.mdx#browser-context-new-page) 來控制它們的確切生命週期。
...
- `options` [Object] *(optional)*
  ...
  - `viewport` [null] | [Object] *(optional)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="browser-new-page-option-viewport"/><a href="#browser-new-page-option-viewport" class="list-anchor">#</a>
    ...
    `null` 值會退出預設預設值，使檢視區依賴作業系統定義的主機視窗大小。這會使測試執行變得不確定。
    :::
...
- `options` [Object] *(optional)*
  - `behavior` "wait" | "ignoreErrors" | "default" *(optional)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="browser-remove-all-listeners-option-behavior"/><a href="#browser-remove-all-listeners-option-behavior" class="list-anchor">#</a>
    ...
    指定是否等待已執行的監聽器以及如果它們拋出錯誤該怎麼辦：
    * `'default'` - 不等待目前的監聽器呼叫（如果有的話）完成，如果監聽器拋出錯誤，可能導致未處理的錯誤
    * `'wait'` - 等待目前的監聽器呼叫（如果有的話）完成
    * `'ignoreErrors'` - 不等待目前的監聽器呼叫（如果有的話）完成，移除後監聽器拋出的所有錯誤都會被靜默捕獲
...
此 API 控制 [Chromium Tracing](https://www.chromium.org/developers/how-tos/trace-event-profiling-tool)，這是一個低階的 Chromium 特定偵錯工具。控制 [Playwright Tracing](../trace-viewer) 的 API 可在[這裡](./class-tracing)找到。
:::

您可以使用 [browser.startTracing()](/api/class-browser.mdx#browser-start-tracing) 和 [browser.stopTracing()](/api/class-browser.mdx#browser-stop-tracing) 來建立可在 Chrome DevTools 效能面板中開啟的追蹤檔案。
...
當 Browser 與瀏覽器應用程式斷開連接時觸發。這可能是由於以下原因之一：
* 瀏覽器應用程式關閉或當機。
* 呼叫了 [browser.close()](/api/class-browser.mdx#browser-close) 方法。
