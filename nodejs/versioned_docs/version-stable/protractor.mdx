...
- Protractor 的 [`waitForAngular`] ⇄ Playwright Test 的 [auto-waiting](./actionability.mdx)
- 在 Playwright Test 中不要忘記使用 await
...
遷移重點（請參閱 Playwright Test 代碼片段中的內聯註釋）：
1. 每個 Playwright Test 文件都有對 `test` 和 `expect` 函數的顯式導入
1. 測試函數標記為 `async`
1. Playwright Test 將 `page` 作為其參數之一。這是 Playwright Test 中許多有用的 [fixtures](./api/class-fixtures) 之一。
1. 幾乎所有 Playwright 調用都以 `await` 為前綴
1. 使用 [page.locator()](/api/class-page.mdx#page-locator) 創建定位器是少數幾個同步方法之一。

## 填充 `waitForAngular`

Playwright Test 具有內建的 [auto-waiting](./actionability.mdx)，這使得在一般情況下不需要 Protractor 的 [`waitForAngular`]。

然而，在某些邊緣情況下，它可能會派上用場。以下是如何在 Playwright Test 中填充 `waitForAngular` 函數：
1. 確保您的 package.json 中安裝了 protractor
1. 填充函數
   ...
   如果您不想保留 protractor 的版本，您也可以使用這個更簡單的方法（僅適用於 Angular 2+）：
...
一旦您使用 Playwright Test，您將獲得很多功能！
- 完整的零配置 TypeScript 支持
- 在 **所有網頁引擎**（Chrome、Firefox、Safari）上運行測試，在 **任何流行操作系統**（Windows、macOS、Ubuntu）上
- 完全支持多個來源、[(i)frames](./api/class-frame)、[tabs 和 contexts](./pages)
- 在多個瀏覽器中並行運行測試
- 內建測試 [artifact collection](./test-use-options.mdx#recording-options)

您還獲得所有這些 ✨ 令人驚嘆的工具 ✨，它們與 Playwright Test 捆綁在一起：
- [Playwright Inspector](./debug.mdx)
- [Playwright Test Code generation](./codegen-intro.mdx)
- [Playwright Tracing](./trace-viewer.mdx) 用於事後調試

## 進一步閱讀

了解更多關於 Playwright Test runner：
- [入門](./intro)
...
[TestWorkerInfo]: /api/class-testworkerinfo.mdx "TestWorkerInfo"
