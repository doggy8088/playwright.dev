...
- Protractor 的 [ElementFinder] ⇄ Playwright Test 的 [Locator](./api/class-locator)
- Protractor 的 [`waitForAngular`] ⇄ Playwright Test 的[自動等待](./actionability.mdx)
- 在 Playwright Test 中別忘了使用 await
...
1. 每個 Playwright Test 檔案都要明確匯入 `test` 與 `expect` 函式
1. 測試函式需標記為 `async`
1. Playwright Test 會透過參數提供 `page`。這是 Playwright Test 眾多[實用的 fixtures](./api/class-fixtures) 之一。
1. 幾乎所有 Playwright 呼叫前都需加上 `await`
1. 使用 [page.locator()](/api/class-page.mdx#page-locator) 建立定位器是少數同步的方法之一。

## 為 `waitForAngular` 提供 Polyfill

Playwright Test 內建[自動等待](./actionability.mdx)，一般情況下不需要 Protractor 的 [`waitForAngular`]。
...
- 完整支援多來源、[(i)frame](./api/class-frame)、[分頁與情境（contexts）](./pages)
- 在多個瀏覽器間並行執行測試
- 內建測試[產物收集](./test-use-options.mdx#recording-options)

此外還有這些 ✨ 超好用工具 ✨ 與 Playwright Test 一起提供：
- [Playwright Inspector](./debug.mdx)
- [Playwright Test 程式碼產生器](./codegen-intro.mdx)
- 用於事後除錯的[Playwright 追蹤](./trace-viewer.mdx)
