...
注意，你需要在狀態過期時刪除儲存的狀態。如果你不需要在測試執行之間保留狀態，請將瀏覽器狀態寫入 [testProject.outputDir](https://ollama.com/api/class-testproject.mdx#test-project-output-dir)，這會在每次測試執行前自動清理。
