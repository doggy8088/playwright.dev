...
本指南說明如何從 [DOM Testing Library](https://testing-library.com/docs/dom-testing-library/intro/)、[React Testing Library](https://testing-library.com/docs/react-testing-library/intro/)、[Vue Testing Library](https://testing-library.com/docs/vue-testing-library/intro) 和 [Svelte Testing Library](https://testing-library.com/docs/svelte-testing-library/intro) 遷移到 Playwright 的[實驗性元件測試](./test-components)。

:::note

若您在瀏覽器中使用 DOM Testing Library（例如，您使用 webpack 來打包端對端測試），您可以直接切換到 Playwright Test。以下範例專注於元件測試，但對於端對端測試，您只需將 `await mount` 替換為 `await page.goto('http://localhost:3000/')` 即可開啟待測頁面。
:::
...
| [queries](https://testing-library.com/docs/queries/about)                       | [定位器](./locators)                                                 |
| [async helpers](https://testing-library.com/docs/dom-testing-library/api-async) | [斷言](./test-assertions)                                        |
| [user events](https://testing-library.com/docs/user-event/intro)                | [操作](./api/class-locator)                                         |
...
1. 對於元件測試，從 `@playwright/experimental-ct-react`（或 -vue、-svelte）匯入所有內容，對於端對端測試，則從 `@playwright/test` 匯入。
1. 測試函數會收到一個與其他測試隔離的 `page`，以及一個在此頁面中渲染元件的 `mount`。這是 Playwright Test 中兩個[有用的佈置](./api/class-fixtures)。
1. 將 `render` 替換為會回傳[元件定位器](./locators)的 `mount`。
1. 使用透過 [locator.locator()](/api/class-locator.mdx#locator-locator) 或 [page.locator()](/api/class-page.mdx#page-locator) 建立的定位器來執行大部分操作。
1. 使用[斷言](./test-assertions)來驗證狀態。

## 遷移查詢

所有查詢，如 `getBy...`、`findBy...`、`queryBy...` 及其多元素對應項，都會被替換為 `component.getBy...` 定位器。定位器總是會自動等待並在需要時重試，因此您不必擔心選擇正確的方法。當您想要執行[清單操作](./locators#lists)，例如斷言文字清單時，Playwright 會自動執行多元素操作。

## 替換 `waitFor`

Playwright 包含會自動等待條件的[斷言](./test-assertions)，因此您通常不需要明確呼叫 `waitFor`/`waitForElementToBeRemoved`。
...
當您找不到合適的斷言時，請改用 [`expect.poll`](./test-assertions#expectpoll)。
...
您可以使用 [locator.locator()](/api/class-locator.mdx#locator-locator) 方法在另一個定位器內建立定位器。
...
- 在**任何熱門作業系統**（Windows、macOS、Ubuntu）上跨**所有網頁引擎**（Chrome、Firefox、Safari）執行測試
- 完整支援多來源、[(i)frames](./api/class-frame)、[分頁和情境](./pages)
- 在多個瀏覽器中平行隔離執行測試
- 內建測試[產出物收集](./test-use-options.mdx#recording-options)

您還能獲得所有這些與 Playwright Test 一同提供的 ✨ 驚人工具 ✨：
- [Visual Studio Code 整合](./getting-started-vscode.mdx)
- [UI 模式](./test-ui-mode.mdx)，用於偵錯測試，具備時光倒流體驗和監看模式。
- [Playwright Inspector](./debug.mdx#playwright-inspector)
- [Playwright Test 程式碼產生](./codegen-intro.mdx)
- [Playwright 追蹤](./trace-viewer.mdx)，用於事後偵錯
