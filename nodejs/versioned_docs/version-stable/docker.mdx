...
2. 針對其他應用程式使用 [browserType.connect()](/api/class-browsertype.mdx#browser-type-connect) API：
...
要在 Docker 內部執行 Playwright，您需要安裝 Node.js、[Playwright 瀏覽器](./browsers.mdx#install-browsers)與[瀏覽器系統相依性](./browsers.mdx#install-system-dependencies)。請參閱以下 Dockerfile：
