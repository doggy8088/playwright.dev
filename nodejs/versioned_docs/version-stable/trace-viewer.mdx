...
在 Actions 標籤中，你可以看到每個動作使用了什麼定位器以及每個動作執行了多長時間。將滑鼠懸停在測試的每個動作上，可以直觀地看到 DOM 快照中的變化。可以在時間上前後移動，並點擊一個動作來檢查和調試。使用 Before 和 After 標籤，可以直觀地看到動作前後發生了什麼。
...
當你點擊側邊欄中的某個動作時，該動作的程式碼行會在源面板中突出顯示。

![顯示程式碼標籤在追蹤檢視器中](https://github.com/microsoft/playwright/assets/13063165/daa8845d-c250-4923-aa7a-5d040da9adc5)

### 呼叫

呼叫標籤顯示有關動作的資訊，例如所花費的時間、使用的定位器、是否處於嚴格模式以及使用了什麼鍵。

![顯示在追蹤檢視器中的呼叫標籤](https://github.com/microsoft/playwright/assets/13063165/95498580-f9dd-4932-a123-c37fe7cfc3c2)
...
Network 標籤顯示了測試期間所做的所有網路請求。你可以按不同類型的請求、狀態碼、方法、請求、內容類型、持續時間和大小進行排序。點擊一個請求以查看更多關於它的資訊，例如請求標頭、回應標頭、請求主體和回應主體。
...
"Attachments" 標籤允許你探索附件。如果你正在進行[視覺回歸測試](./test-snapshots.mdx)，你將能夠通過檢查圖像差異、實際圖像和預期圖像來比較截圖。當你點擊預期圖像時，你可以使用滑桿將一個圖像滑動到另一個圖像上，以便輕鬆看到截圖中的差異。

![附件標籤在追蹤查看器中](https://github.com/microsoft/playwright/assets/13063165/4386178a-5808-4fa8-9436-315350a23b04)

## 在本地記錄追蹤

要在開發模式下記錄追蹤，請在執行測試時將 `--trace` 標誌設置為 `on`。你也可以使用 [UI Mode](./test-ui-mode.mdx) 來獲得更好的開發者體驗。
...
有更多細粒度的選項可用，請參見 [testOptions.trace](https://ollama.com/api/class-testoptions.mdx#test-options-trace)。

如果你沒有使用 Playwright 作為測試執行器，請改用 [browserContext.tracing](/api/class-browsercontext.mdx#browser-context-tracing) API。
