...
要在開發模式下記錄追蹤，請在執行測試時將 `--trace` 旗標設置為 `on`。你也可以使用 [UI Mode](./test-ui-mode.mdx) 來獲得更好的開發者體驗。
...
有更多細粒度的選項可用，請參見 [testOptions.trace](/api/class-testoptions.mdx#test-options-trace)。
