...
| `-c <file>` or `--config <file>`| 配置文件。如果未傳遞，默認為當前目錄中的 `playwright.config.ts` 或 `playwright.config.js`。 |
| `--debug`| 使用 Playwright Inspector 執行測試。是 `PWDEBUG=1` 環境變數和 `--timeout=0 --max-failures=1 --headed --workers=1` 選項的快捷方式。|
| `--fail-on-flaky-tests` | 包含不穩定測試的測試運行失敗。默認情況下，不穩定的測試計算為成功。 |
| `--forbid-only` | 是否禁止 `test.only`。在 CI 上很有用。|
| `--global-timeout <number>` | 整個測試運行的總超時時間（毫秒）。默認情況下，沒有全域超時。了解更多關於[各種超時](./test-timeouts.mdx)。|
...
| `--output <dir>` | 測試生成的工件目錄，默認為 `test-results`。 |
| `--only-changed [ref]` | 只運行工作樹和 "ref" 之間已更改的測試文件。默認為運行所有未提交的更改，ref=HEAD。僅支持 Git。 |
| `--pass-with-no-tests` | 當未找到文件時允許測試套件通過。 |
| `--project <name>` | 只運行指定[專案](./test-projects.mdx)的測試，支持 '*' 通配符。默認為運行配置文件中定義的所有專案。|
| `--quiet` | 是否抑制測試的 stdout 和 stderr。 |
| `--repeat-each <N>` | 每個測試運行 `N` 次，默認為一次。 |
...
| `--retries <number>` | 不穩定測試的最大[重試](./test-retries.mdx#retries)次數，默認為零（不重試）。 |
| `--shard <shard>` | [分片](./test-parallel.mdx#shard-tests-between-multiple-machines)測試並僅執行選定的分片，以 `current/all` 的形式指定，從 1 開始，例如 `3/5`。|
| `--timeout <number>` | 每個測試的最大超時時間（毫秒），默認為 30 秒。了解更多關於[各種超時](./test-timeouts.mdx)。|
