...
你也可以使用 [VS Code Extension](./getting-started-vscode.mdx) 來產生定位器以及錄製測試。VS Code extension 也讓你在撰寫、執行和偵錯測試時擁有極佳的開發者體驗。
...
本地偵錯我們建議你通過安裝 [VS Code extension](./getting-started-vscode.mdx) [在 VSCode 中即時偵錯你的測試](./getting-started-vscode.mdx#live-debugging)。你可以通過右鍵點擊你想要執行的測試旁邊的行來在偵錯模式下執行測試，這將打開一個瀏覽器窗口並在設置斷點處暫停。
...
你也可以使用 Playwright 檢查器來偵錯你的測試，方法是使用 `--debug` 標誌來執行你的測試。
...
要調試特定測試，請添加測試檔案的名稱和測試的行號，並加上 `--debug` 標誌。
...
Playwright 配置檔案中配置了追蹤，並設定在 CI 上第一次重試失敗測試時執行。我們不建議將其設定為 `on`，以便在每次測試時都執行追蹤，因為這會非常影響效能。然而，你可以在開發時使用 `--trace` 標誌在本地執行追蹤。
...
- [VS Code extension](./getting-started-vscode.mdx) 在撰寫、執行和偵錯測試時為您提供了極佳的開發者體驗。
- [test generator](./codegen.mdx) 可以為您生成測試並選擇定位器。
- [trace viewer](./trace-viewer.mdx) 為您提供測試的完整追蹤，作為本地 PWA，可以輕鬆分享。使用 trace viewer，您可以查看時間軸、檢查每個動作的 DOM 快照、查看網路請求等。
- [UI Mode](./test-ui-mode) 讓您以時間旅行的體驗來探索、執行和偵錯測試，並配有觀察模式。所有測試檔案都會載入測試側邊欄，您可以展開每個檔案和描述區塊來單獨執行、查看、觀察和偵錯每個測試。
...
通過保持你的 Playwright 版本為最新，你將能夠在最新的瀏覽器版本上測試你的應用程式，並在最新的瀏覽器版本公開發布之前捕捉到失敗。
