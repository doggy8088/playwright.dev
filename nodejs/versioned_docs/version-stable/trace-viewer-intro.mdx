...
要了解更多關於記錄追蹤的可用選項，請查看我們在 [Trace Viewer](/trace-viewer.mdx) 上的詳細指南。

追蹤通常在持續整合（CI）環境中執行，因為在本地你可以使用 [UI Mode](/test-ui-mode.mdx) 來開發和除錯測試。然而，如果你想在本地執行追蹤而不使用 [UI Mode](/test-ui-mode.mdx)，你可以使用 `--trace on` 強制開啟追蹤。
...
要了解更多關於追蹤的資訊，請查看我們的[追蹤檢視器](/trace-viewer.mdx)詳細指南。
