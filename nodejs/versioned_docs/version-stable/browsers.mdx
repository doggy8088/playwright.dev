...
透過保持 Playwright 版本為最新，您將能夠使用新功能並在最新瀏覽器版本上測試您的應用程式，並在最新瀏覽器版本向公眾發布之前捕捉到失敗。
...
請查看[發布說明](./release-notes.mdx)以了解最新版本以及已發布的變更。

```bash
# 透過執行下列指令查看您擁有的 Playwright 版本
...
Playwright 可以透過在設定中設定**專案**來在多個瀏覽器和配置中執行您的測試。您也可以為每個專案新增[不同的選項](./test-configuration)。
...
執行使用 1 個 worker 的 1 個測試

  ✓ [firefox] › example.spec.ts:3:1 › basic test (2s)
```

使用 VS Code 擴充套件時，您可以透過勾選 Playwright 側邊欄中瀏覽器名稱旁邊的複選框來在不同的瀏覽器上執行測試。這些名稱在 Playwright 設定檔案的專案區段中定義。安裝 Playwright 時的預設設定提供了 3 個專案：Chromium、Firefox 和 WebKit。預設選擇第一個專案。

<ProgressiveImage image={require("../images/vscode-projects-section.png")} alt="VS Code 擴充套件中的專案區段" />

要在多個專案（瀏覽器）上執行測試，請透過勾選專案名稱旁邊的複選框來選擇每個專案。
...
`PLAYWRIGHT_BROWSERS_PATH` 不會改變 Google Chrome 和 Microsoft Edge 的安裝路徑。
:::
