...
默認情況下，如果某些測試失敗，HTML 報告會自動打開。你可以通過 Playwright 配置中的 `open` 屬性或 `PLAYWRIGHT_HTML_OPEN` 環境變數來控制此行為。該屬性的可能值為 `always`、`never` 和 `on-failure`（默認）。
