...
我們建議使用 [VS Code Extension](./getting-started-vscode.mdx) 來進行偵錯，以獲得更好的開發者體驗。有了 VS Code extension，你可以直接在 VS Code 中偵錯你的測試，查看錯誤訊息，設置斷點並逐步執行你的測試。
...
要設定中斷點，請點擊您希望中斷點所在行號旁邊，直到出現一個紅點。通過右鍵點擊您希望執行的測試旁邊的行，在調試模式下執行測試。
...
默認情況下，調試是使用 Chromium 配置檔進行的。您可以通過右鍵單擊測試側邊欄中的調試圖標，然後從下拉選單中單擊 'Select Default Profile' 選項，在不同的瀏覽器上調試您的測試。
...
然後選擇您想要用來調試測試的測試配置檔。每次在調試模式下執行測試時，它將使用您選擇的配置檔。您可以通過右鍵點擊測試所在的行號，並從菜單中選擇 'Debug Test' 來在調試模式下執行測試。

<img width="1312" alt="選擇一個調試配置" src="https://user-images.githubusercontent.com/13063165/212880198-eac22c3e-68ce-47da-9163-d6b376ae7575.png" />

要了解更多關於除錯的資訊，請參見 [Debugging in Visual Studio Code](https://code.visualstudio.com/docs/editor/debugging)。

## Playwright Inspector

Playwright Inspector 是一個 GUI 工具，可以幫助你調試 Playwright 測試。它允許你逐步執行測試、即時編輯定位器、選擇定位器並查看可操作性日誌。
...
執行你的測試並使用 `--debug` 標誌來開啟檢查器。這會配置 Playwright 進行偵錯並開啟檢查器。當使用 `--debug` 時，會配置其他有用的預設值:
...
要調試所有測試，請使用 `--debug` 標誌執行測試命令。這將逐一執行測試，並為每個測試打開檢查器和瀏覽器窗口。
...
要在特定行上偵錯一個測試，請執行測試命令，後跟測試檔案的名稱和您要偵錯的測試行號，然後是 `--debug` 標誌。這將在每個配置在您的 [`playwright.config`](./test-projects.mdx#configure-projects-for-multiple-browsers) 中的瀏覽器中執行單個測試並打開檢查器。
...
在 Playwright 中，你可以在你的[`playwright.config`](./test-projects.mdx#configure-projects-for-multiple-browsers)中配置專案。一旦配置完成，你可以使用 `--project` 標誌，後接在你的 `playwright.config` 中配置的專案名稱，來在特定的瀏覽器或行動裝置視窗中調試你的測試。
...
要在特定瀏覽器上執行一個測試，請添加測試檔案的名稱和您要調試的測試行號，以及 `--project` 標誌，後面跟著專案的名稱。
...
你也可以使用 `slowMo` 選項來減慢執行速度（每次操作 N 毫秒），並在調試時跟隨。
