...
要偵錯所有測試，請使用 `--debug` 標誌執行測試命令。這將逐一執行測試，並為每個測試打開檢查器和瀏覽器窗口。
...
在 Playwright 中，你可以在你的[`playwright.config`](./test-projects.mdx#configure-projects-for-multiple-browsers)中配置專案。一旦配置完成，你可以使用 `--project` 標誌，後接在你的 `playwright.config` 中配置的專案名稱，來在特定的瀏覽器或行動裝置視窗中偵錯你的測試。
...
要在特定瀏覽器上執行一個測試，請添加測試檔案的名稱和您要偵錯的測試行號，以及 `--project` 標誌，後面跟著專案的名稱。
