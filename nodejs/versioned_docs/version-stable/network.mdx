...
1. 如果你正在使用 Playwright 的原生 [browserContext.route()](/api/class-browsercontext.mdx#browser-context-route) 和 [page.route()](/api/class-page.mdx#page-route)，並且出現網路事件遺失的情況，請通過將 [serviceWorkers](/api/class-browser.mdx#browser-new-context-option-service-workers) 設定為 `'block'` 來停用 Service Workers。
