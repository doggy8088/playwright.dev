...
UI 模式和追蹤檢視器中的 Network 標籤有幾個不錯的改進:
...
默認情況下，Playwright 將使用啟發式方法查找每個匯入文件的最近 tsconfig。您現在可以在命令列中指定一個 tsconfig 文件，Playwright 將對所有匯入文件使用它，而不僅僅是測試文件:
...
- 現在可以在 HTML 報告中的新標籤頁中打開 `text/html` 內容類型的附件。這對於在 Playwright 測試報告中包含第三方報告或其他 HTML 內容並將其分發給您的團隊非常有用。
...
- 新設定可顯示/隱藏路由操作，如 [route.continue()](https://ollama.com/api/class-route.mdx#route-continue)。
- 請求方法和狀態顯示在網路詳細資訊標籤中。
- 新按鈕可將來源檔案位置複製到剪貼簿。
...
- 新的 CLI 選項 `--fail-on-flaky-tests`，在任何不穩定的測試時將退出碼設置為 `1`。請注意，默認情況下，當所有失敗的測試在重試後恢復時，測試運行器會以退出碼 `0` 退出。使用此選項，在這種情況下測試運行將失敗。
...
- Method [locator.setInputFiles()](https://api/class-locator.mdx#locator-set-input-files) now supports uploading a directory for `<input type=file webkitdirectory>` elements。

  ```ts
  await page.getByLabel('Upload directory').setInputFiles(path.join(__dirname, 'mydir'));
  ```

- Multiple methods like [locator.click()](https://api/class-locator.mdx#locator-click) or [locator.press()](https://api/class-locator.mdx#locator-press) now support a `ControlOrMeta` modifier key. This key maps to `Meta` on macOS and maps to `Control` on Windows and Linux。
...
- New property `httpCredentials.send` in [apiRequest.newContext()](https://api/class-apirequest.mdx#api-request-new-context) that allows to either always send the `Authorization` header or only send it in response to `401 Unauthorized`。
- New option `reason` in [apiRequestContext.dispose()](https://api/class-apirequestcontext.mdx#api-request-context-dispose) that will be included in the error message of ongoing operations interrupted by the context disposal。
- New option `host` in [browserType.launchServer()](https://api/class-browsertype.mdx#browser-type-launch-server) allows to accept websocket connections on a specific address instead of unspecified `0.0.0.0`。
...
- [expect(locator).toHaveAccessibleName()](https://api/class-locatorassertions.mdx#locator-assertions-to-have-accessible-name) 檢查元素是否具有指定的可存取名稱:
...
- [expect(locator).toHaveAccessibleDescription()](https://api/class-locatorassertions.mdx#locator-assertions-to-have-accessible-description) 檢查元素是否具有指定的可存取描述:
...
- [expect(locator).toHaveRole()](https://api/class-locatorassertions.mdx#locator-assertions-to-have-role) 檢查元素是否具有指定的 ARIA 角色:
...
- 新方法 [suite.entries()](https://ollama.com/api/class-suite.mdx#suite-entries) 返回子測試套件和測試案例的宣告順序。 [suite.type](https://ollama.com/api/class-suite.mdx#suite-type) 和 [testCase.type](https://ollama.com/api/class-testcase.mdx#test-case-type) 可用於區分清單中的測試案例和套件。
- [Blob](https://ollama.com/test-reporters#blob-reporter) 報告器現在允許使用單一選項 `outputFile` 覆蓋報告檔案路徑。相同的選項也可以指定為 `PLAYWRIGHT_BLOB_OUTPUT_FILE` 環境變數，這在 CI/CD 上可能更方便。
- [JUnit](https://ollama.com/test-reporters#junit-reporter) 報告器現在支援 `includeProjectInTestName` 選項。
...
- 新方法 [page.addLocatorHandler()](https://ollama.com/library#page-add-locator-handler) 註冊了一個回呼，當指定的元素變得可見時將會被呼叫並可能阻止 Playwright 操作。回呼可以去除覆蓋層。這裡有一個範例，當 cookie 對話框出現時將其關閉:
...
- [新語法](./test-annotations#annotate-tests) 用於測試註解:
...
- [page.pdf()](https://api/class-page.mdx#page-pdf) 接受兩個新選項 [`tagged`](./api/class-page#page-pdf-option-tagged) 和 [`outline`](./api/class-page#page-pdf-option-outline)。
...
請參閱 [test.step()](https://api/class-test.mdx#test-step) 文件以獲取完整範例。
...
- 新選項 `hasNot` 在 [locator.filter()](https://api/class-locator.mdx#locator-filter)
- 新選項 `hasNotText` 在 [locator.filter()](https://api/class-locator.mdx#locator-filter)
- [expect(locator).toBeAttached()](https://api/class-locatorassertions.mdx#locator-assertions-to-be-attached)
- 新選項 `timeout` 在 [route.fetch()](https://api/class-route.mdx#route-fetch)
- [reporter.onExit()](https://api/class-reporter.mdx#reporter-on-exit)
...
- [locator.blur()](https://ollama.com/api/class-locator.mdx#locator-blur)
- [locator.clear()](https://ollama.com/api/class-locator.mdx#locator-clear)
- [android.launchServer()](https://ollama.com/api/class-android.mdx#android-launch-server) 和 [android.connect()](https://ollama.com/api/class-android.mdx#android-connect)
- [androidDevice.on('close')](https://ollama.com/api/class-androiddevice.mdx#android-device-event-close)
...
- [expect(locator).toHaveAttribute()](https://api/class-locatorassertions.mdx#locator-assertions-to-have-attribute) with an empty value does not match missing attribute anymore. 例如, 當 `button` **沒有** `disabled` 屬性時，以下程式碼片段將會成功。
...
- 新選項 `maxRedirects` 用於 [apiRequestContext.get()](https://ollama.com/library#api-request-context-get) 和其他函式以限制重定向次數。
...
* [test.step()](https://api/class-test.mdx#test-step) 現在返回 step 函式的值:
...
* 新增 [test.describe.fixme()](https://api/class-test.mdx#test-describe-fixme)。
* 新的 `'interrupted'` 測試狀態。
* 通過 CLI 標誌啟用追蹤: `npx playwright test --trace=on`。
...
現在可以呼叫 [test.describe()](https://api/class-test.mdx#test-describe) 來建立沒有標題的套件。這對於使用 [test.use()](https://api/class-test.mdx#test-use) 為一組測試提供共同選項很有用。
...
注意，新方法 [page.routeFromHAR()](https://api/class-page.mdx#page-route-from-har) 和 [browserContext.routeFromHAR()](https://api/class-browsercontext.mdx#browser-context-route-from-har) 也參與路由並且可以被延遲。

### Web-First Assertions Update

* 新方法 [expect(locator).toHaveValues()](https://api/class-locatorassertions.mdx#locator-assertions-to-have-values) 可斷言 `<select multiple>` 元素的所有選定值。
* 方法 [expect(locator).toContainText()](https://api/class-locatorassertions.mdx#locator-assertions-to-contain-text) 和 [expect(locator).toHaveText()](https://api/class-locatorassertions.mdx#locator-assertions-to-have-text) 現在接受 `ignoreCase` 選項。
...
* 如果有服務工作者阻礙了你的操作，你現在可以使用新的上下文選項 `serviceWorkers` 輕鬆禁用它:
...
* Playwright 現在可以在 Ubuntu 22 amd64 和 Ubuntu 22 arm64 上執行。我們還發布了新的 docker 映像 `mcr.microsoft.com/playwright:v1.34.0-jammy`。
...
- [page.screenshot()](./api/class-page.mdx#page-screenshot) 中的新 `caret` 選項，用於控制文本插入符號。默認為 `"hide"`。
...
- Playwright Test 現在添加了 [testConfig.fullyParallel](/api/class-testconfig.mdx#test-config-fully-parallel) 模式。默認情況下，Playwright Test 在文件之間並行。在完全並行模式下，單個文件內的測試也會並行執行。您還可以使用 `--fully-parallel` 命令行標誌。
...
我們注意到，在少數情況下，要執行的測試集是通過環境變數在全域設定中配置的。我們還注意到一些應用程式在全域拆解中後處理報告者的輸出。如果你正在做其中之一，[了解更多](https://github.com/microsoft/playwright/issues/12018)。
...
閱讀更多關於[我們的報告者](./test-reporters#html-reporter)。
...
使用 `npx playwright test --debug` 將啟用 [Playwright Inspector](./debug#playwright-inspector) 以便您調試測試。
...
#### ⛓ 串行模式與 [`describe.serial`](./api/class-test#test-describe-serial)

宣告一組應該始終串行執行的測試。如果其中一個測試失敗，所有後續測試都會被跳過。組中的所有測試會一起重新嘗試。
...
- [Playwright 測試註解](./test-annotations.mdx)
...
- 新的 `baseURL` 選項在 [browser.newContext()](https://ollama.com/api/class-browser.mdx#browser-new-context) 和 [browser.newPage()](https://ollama.com/api/class-browser.mdx#browser-new-page)
- [response.securityDetails()](https://ollama.com/api/class-response.mdx#response-security-details) 和 [response.serverAddr()](https://ollama.com/api/class-response.mdx#response-server-addr)
- [page.dragAndDrop()](https://ollama.com/api/class-page.mdx#page-drag-and-drop) 和 [frame.dragAndDrop()](https://ollama.com/api/class-frame.mdx#frame-drag-and-drop)
- [download.cancel()](https://ollama.com/api/class-download.mdx#download-cancel)
- [page.inputValue()](https://ollama.com/api/class-page.mdx#page-input-value), [frame.inputValue()](https://ollama.com/api/class-frame.mdx#frame-input-value) 和 [elementHandle.inputValue()](https://ollama.com/api/class-elementhandle.mdx#element-handle-input-value)
- 新的 `force` 選項在 [page.fill()](https://ollama.com/api/class-page.mdx#page-fill), [frame.fill()](https://ollama.com/api/class-frame.mdx#frame-fill), 和 [elementHandle.fill()](https://ollama.com/api/class-elementhandle.mdx#element-handle-fill)
- 新的 `force` 選項在 [page.selectOption()](https://ollama.com/api/class-page.mdx#page-select-option), [frame.selectOption()](https://ollama.com/api/class-frame.mdx#frame-select-option), 和 [elementHandle.selectOption()](https://ollama.com/api/class-elementhandle.mdx#element-handle-select-option)
