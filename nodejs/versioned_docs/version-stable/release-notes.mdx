...
### 新的 API
- 新Property [testStepInfo.titlePath](/api/class-teststepinfo.mdx#test-step-info-title-path) Returns the full title path starting 從 測試檔案, 包括 test and step titles.
...
- 新選項 `noSnippets` to disable code snippets 在 html report 中.
...
- 新屬性 `location` in test annotations, 例如 in [testResult.annotations](/api/class-testresult.mdx#test-result-annotations) and [testInfo.annotations](/api/class-testinfo.mdx#test-info-annotations). It shows where the annotation like `test.skip` or `test.fixme` was added.
...
### 追蹤檢視器和 HTML 報告更新
- 新Steps in Trace Viewer and HTML reporter: ![New Trace Viewer Steps](https://github.com/user-attachments/assets/1963ff7d-4070-41be-a79b-4333176921a2)
