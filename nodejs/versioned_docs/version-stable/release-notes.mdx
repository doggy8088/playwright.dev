...
- 新的環境變量 `PLAYWRIGHT_FORCE_TTY` 控制內建的 `list`、`line` 和 `dot` 報告器是否假設為即時終端。例如，當您的 CI 環境無法很好地處理 ANSI 控制序列時，這可能有助於禁用 tty 行為。或者，如果您計劃後處理輸出並處理控制序列，即使沒有即時終端，也可以啟用 tty 行為。
