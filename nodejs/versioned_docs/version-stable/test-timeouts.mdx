...
默認情況下，[fixture](./test-fixtures) 與測試共享超時時間。然而，對於慢速的 fixtures，特別是[worker-scoped](./test-fixtures#worker-scoped-fixtures) 的那些，擁有單獨的超時時間會更方便。這樣你可以保持整體測試的超時時間較短，並給慢速的 fixture 更多時間。
