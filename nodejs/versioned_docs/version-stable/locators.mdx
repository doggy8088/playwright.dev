...
XPath 和 CSS 選擇器可能與 DOM 結構或實作緊密相關。當 DOM 結構改變時，這些選擇器可能會失效。下面的長 CSS 或 XPath 鏈是導致不穩定測試的**不良實踐**範例：
...
Playwright 中的所有定位器**預設**都能與 Shadow DOM 中的元素一起使用。例外情況如下：
- 使用 XPath 定位不會穿透 shadow root。
...
* 這會找到兩個按鈕並拋出[嚴格性](./locators.mdx#strictness)違規錯誤：
