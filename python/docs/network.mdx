...
等待 [Response]s with [page.expect_response()](/api/class-page.mdx#page-wait-for-response)
...
在整個瀏覽器上下文中設定路由，使用 [browser_context.route()](/api/class-browsercontext.mdx#browser-context-route) 或頁面使用 [page.route()](/api/class-page.mdx#page-route)。它將適用於彈出視窗和打開的連結。
