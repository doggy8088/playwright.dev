...
公開可用於 Web API 測試的 API。此類別用於建立 [APIRequestContext] 執行個體，該執行個體可用於傳送 Web 請求。 An instance of this class can be obtained via [playwright.request](/api/class-playwright.mdx#playwright-request). For more information see [APIRequestContext].
