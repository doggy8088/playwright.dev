...
- `on`：為每個測試記錄追蹤
- `off`：不記錄追蹤。（預設）
- `retain-on-failure`：為每個測試記錄追蹤，但從成功的測試執行中移除所有追蹤。

這將記錄追蹤並將其放置到您的 `test-results` 目錄中名為 `trace.zip` 的檔案中。

<details>

<summary>如果您沒有使用 Pytest，請點擊這裡了解如何記錄追蹤。</summary>
