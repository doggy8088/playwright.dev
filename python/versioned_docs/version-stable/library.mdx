...
您很可能不需要手動等待，因為 Playwright 具有[自動等待](./actionability.mdx)功能。如果您仍然依賴它，您應該使用 `page.wait_for_timeout(5000)` 而不是 `time.sleep(5)`，最好根本不等待逾時，但有時對於偵錯很有用。在這些情況下，請使用我們的等待（`wait_for_timeout`）方法而不是 `time` 模組。這是因為我們內部依賴非同步操作，當使用 `time.sleep(5)` 時，它們無法正確處理。

### 與 Windows 上 `asyncio` 的 `SelectorEventLoop` 不相容
