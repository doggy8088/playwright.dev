...
等待 [Response]s 使用 [page.expect_response()](https://api/class-page.mdx#page-wait-for-response)
...
在整個瀏覽器上下文中使用 [browser_context.route()](https://ollama.com/api/class-browsercontext.mdx#browser-context-route) 或頁面使用 [page.route()](https://ollama.com/api/class-page.mdx#page-route) 設定路由。它將應用於彈出窗口和打開的連結。
...
Playwright 的內建 [browser_context.route()](https://api/class-browsercontext.mdx#browser-context-route) 和 [page.route()](https://api/class-page.mdx#page-route) 允許你的測試原生地路由請求並執行模擬和攔截。

1. 如果你正在使用 Playwright 的原生 [browser_context.route()](/api/class-browsercontext.mdx#browser-context-route) 和 [page.route()](/api/class-page.mdx#page-route)，並且似乎缺少網路事件，請通過將 `browser.new_context.service_workers` 設置為 `'block'` 來禁用 Service Workers。
