...
:::note
如果頁面在 `load` 之前進行客戶端重定向，[Page.GotoAsync()](/api/class-page.mdx#page-goto) 會等待重定向的頁面觸發 `load` 事件。
:::
...
點擊一個元素可能會觸發多次導航。在這些情況下，建議明確地 [Page.WaitForURLAsync()](https://ollama.com/library) 到一個特定的 URL。
...
- [Page.Url](/api/class-page.mdx#page-url) 被設定為新 url
- 文件內容通過網路加載並解析
- [Page.DOMContentLoaded](https://api/class-page.mdx#page-event-dom-content-loaded) 事件被觸發
- 頁面執行一些腳本並加載樣式表和圖像等資源
- [Page.Load](https://api/class-page.mdx#page-event-load) 事件被觸發
- 頁面執行動態加載的腳本
