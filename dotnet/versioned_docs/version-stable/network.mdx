...
或者在按鈕點擊後等待網路回應 [Page.RunAndWaitForResponseAsync()](https://ollama.com/api/class-page.mdx#page-wait-for-response):
...
等待 [Response]s with [Page.RunAndWaitForResponseAsync()](https://ollama.com/api/class-page.mdx#page-wait-for-response)
...
Playwright 的內建 [BrowserContext.RouteAsync()](https://api/class-browsercontext.mdx#browser-context-route) 和 [Page.RouteAsync()](https://api/class-page.mdx#page-route) 允許您的測試原生地路由請求並執行模擬和攔截。
