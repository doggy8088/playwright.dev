...
除了推薦的定位器如 [Page.GetByRole()](/api/class-page.mdx#page-get-by-role) 和 [Page.GetByText()](/api/class-page.mdx#page-get-by-text) 外，Playwright 支援本指南中描述的各種其他定位器。
...
請注意，結果匹配是根據它們與錨點元素的距離排序的，因此您可以使用 [Locator.First](/api/class-locator.mdx#locator-first) 來選擇最近的一個。如果您有類似元素的列表，其中最近的顯然是正確的，這才有用。然而，在其他情況下使用 [Locator.First](/api/class-locator.mdx#locator-first) 很可能不會如預期般工作——它不會定位到您正在搜索的元素，而是一些碰巧最近的其他元素，如隨機的空 `<div>`，或是滾動出來且當前不可見的元素。
...
`:nth-match()` 也可用於等待直到出現指定數量的元素，使用 [Locator.WaitForAsync()](/api/class-locator.mdx#locator-wait-for)。
...
當你需要針對某個元素的父元素時，大多數情況下你應該通過子定位器 [Locator.Filter()](/api/class-locator.mdx#locator-filter)。例如，考慮以下 DOM 結構:
...
如果你想針對帶有文字 "Hello" 的標籤的父 `<li>`，使用 [Locator.Filter()](/api/class-locator.mdx#locator-filter) 最有效：
...
或者，如果您無法找到適合的父元素定位器，請使用 `xpath=..`。請注意，這種方法不太可靠，因為對 DOM 結構的任何更改都會破壞您的測試。盡可能偏好使用 [Locator.Filter()](/api/class-locator.mdx#locator-filter)。
...
例如，考慮以下 DOM 結構: `<label for="password">Password:</label><input id="password" type="password">`。你可以使用 [Page.GetByText()](/api/class-page.mdx#page-get-by-text) 根據其 "Password" 文字來定位標籤。然而，以下操作將在 input 上執行而不是在標籤上:

- [Locator.ClickAsync](https://ollama.com/api/class-locator.mdx#locator-click)() 會點擊標籤並自動聚焦到輸入欄位;
- [Locator.FillAsync](https://ollama.com/api/class-locator.mdx#locator-fill)() 會填充輸入欄位;
- [Locator.InputValueAsync](https://ollama.com/api/class-locator.mdx#locator-input-value)() 會返回輸入欄位的值;
- [Locator.SelectTextAsync](https://ollama.com/api/class-locator.mdx#locator-select-text)() 會選擇輸入欄位中的文字;
- [Locator.SetInputFilesAsync](https://ollama.com/api/class-locator.mdx#locator-set-input-files)() 會為 `type=file` 的輸入欄位設置文件;
- [Locator.SelectOptionAsync](https://ollama.com/api/class-locator.mdx#locator-select-option)() 會從選擇框中選擇一個選項。
...
然而，其他方法將針對標籤本身，例如 [Expect(Locator).ToHaveTextAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-have-text) 會斷言標籤的文字內容，而不是輸入欄位。
