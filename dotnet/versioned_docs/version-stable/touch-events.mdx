...
處理舊版 [觸控事件](https://developer.mozilla.org/en-US/docs/Web/API/Touch_events) 以回應滑動、縮放和點擊等手勢的網頁應用程式，可以透過手動分派 [TouchEvent](https://developer.mozilla.org/en-US/docs/Web/API/TouchEvent/TouchEvent) 到頁面來進行測試。以下範例說明如何使用 [`Locator.DispatchEventAsync()`](/api/class-locator.mdx#locator-dispatch-event) 並將 [Touch](https://developer.mozilla.org/en-US/docs/Web/API/Touch) 點作為引數傳遞。

請注意，[`Locator.DispatchEventAsync()`](/api/class-locator.mdx#locator-dispatch-event) 不會設定 [`Event.isTrusted`](https://developer.mozilla.org/en-US/docs/Web/API/Event/isTrusted) 屬性。如果您的網頁依賴此屬性，請確保在測試期間停用 `isTrusted` 檢查。
