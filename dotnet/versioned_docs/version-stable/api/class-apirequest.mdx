...
公開可用於 Web API 測試的 API。此類別用於建立 `APIRequestContext` 實例，進而可用於傳送 Web 請求。此類別的實例可透過 [Playwright.APIRequest](/api/class-playwright.mdx#playwright-request) 取得。如需更多資訊，請參閱 `APIRequestContext`。
...
- `options` `ApiRequestNewContextOptions?` *(optional)*
  - `BaseURL` [string]? *(optional)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="api-request-new-context-option-base-url"/><a href="#api-request-new-context-option-base-url" class="list-anchor">#</a>
    ...
    Methods like [ApiRequestContext.GetAsync()](/api/class-apirequestcontext.mdx#api-request-context-get) 會透過使用 [`URL()`](https://developer.mozilla.org/en-US/docs/Web/API/URL/URL) 建構函式來建立對應的 URL，將基礎 URL 納入考慮。範例：
  ...
  - `HttpCredentials` HttpCredentials? *(optional)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="api-request-new-context-option-http-credentials"/><a href="#api-request-new-context-option-http-credentials" class="list-anchor">#</a>
    ...
    [HTTP 驗證](https://developer.mozilla.org/en-US/docs/Web/HTTP/Authentication). 如果沒有指定來源，使用者名稱和密碼會在未授權回應時傳送給任何伺服器。
  ...
  - `StorageState` [string]? *(optional)*<a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="api-request-new-context-option-storage-state"/><a href="#api-request-new-context-option-storage-state" class="list-anchor">#</a>
    ...
    使用給定的儲存狀態填入情境。此選項可用於使用透過 [BrowserContext.StorageStateAsync()](/api/class-browsercontext.mdx#browser-context-storage-state) 或 [ApiRequestContext.StorageStateAsync()](/api/class-apirequestcontext.mdx#api-request-context-storage-state) 取得的已登入資訊來初始化情境。可以是儲存檔案的路徑，或是 [BrowserContext.StorageStateAsync()](/api/class-browsercontext.mdx#browser-context-storage-state) 或 [ApiRequestContext.StorageStateAsync()](/api/class-apirequestcontext.mdx#api-request-context-storage-state) 方法傳回的值。
  - `StorageStatePath` [string]? *(optional)* <font size="2">Added in: v1.18</font><a aria-hidden="true" tabIndex="-1" class="list-anchor-link" id="api-request-new-context-option-storage-state-path"/><a href="#api-request-new-context-option-storage-state-path" class="list-anchor">#</a>
    ...
    使用給定的儲存狀態填入情境。此選項可用於使用透過 [BrowserContext.StorageStateAsync()](/api/class-browsercontext.mdx#browser-context-storage-state) 取得的已登入資訊來初始化情境。已儲存儲存狀態的檔案路徑。
