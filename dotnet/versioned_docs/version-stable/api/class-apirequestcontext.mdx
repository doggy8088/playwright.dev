...
每個 Playwright 瀏覽器情境都有與其關聯的 [APIRequestContext] 實例，該實例與瀏覽器情境共享 cookie 儲存，並可透過 [BrowserContext.APIRequest](/api/class-browsercontext.mdx#browser-context-request) 或 [Page.APIRequest](/api/class-page.mdx#page-request) 存取。也可以透過呼叫 [ApiRequest.NewContextAsync()](/api/class-apirequest.mdx#api-request-new-context) 手動建立新的 APIRequestContext 實例。

**Cookie 管理**

由 [BrowserContext.APIRequest](/api/class-browsercontext.mdx#browser-context-request) 和 [Page.APIRequest](/api/class-page.mdx#page-request) 回傳的 [APIRequestContext] 與對應的 [BrowserContext] 共享 cookie 儲存。每個 API 請求都會使用來自瀏覽器情境的值填入 `Cookie` 標頭。如果 API 回應包含 `Set-Cookie` 標頭，它會自動更新 [BrowserContext] cookie，並且從頁面發出的請求會選取它們。這意味著如果您使用此 API 登入，您的端對端測試將會被登入，反之亦然。

如果您希望 API 請求不干擾瀏覽器 cookie，您應該透過呼叫 [ApiRequest.NewContextAsync()](/api/class-apirequest.mdx#api-request-new-context) 建立新的 [APIRequestContext]。這樣的 `APIRequestContext` 物件將擁有自己的隔離 cookie 儲存。
...
建立新的 [FormData] 實例，用於在發出 HTTP 請求時提供表單和多部分資料。
