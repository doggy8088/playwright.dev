...
每個 Playwright 瀏覽器情境都有一個關聯的 [APIRequestContext] 執行個體，該執行個體與瀏覽器情境共享 cookie 儲存，可透過 [BrowserContext.APIRequest](/api/class-browsercontext.mdx#browser-context-request) 或 [Page.APIRequest](/api/class-page.mdx#page-request) 存取。也可以透過呼叫 [ApiRequest.NewContextAsync()](/api/class-apirequest.mdx#api-request-new-context) 手動建立新的 APIRequestContext 執行個體。
...
如果您希望 API 請求不會干擾瀏覽器 cookie，您應該透過呼叫 [ApiRequest.NewContextAsync()](/api/class-apirequest.mdx#api-request-new-context) 建立新的 [APIRequestContext]。這樣的 `APIRequestContext` 物件將擁有自己獨立的 cookie 儲存。
...
建立新的 [FormData] 執行個體，用於在發出 HTTP 請求時提供表單和 multipart 資料。
