...
要修改回應，使用 [APIRequestContext] 取得原始回應，然後將回應傳遞給 [Route.FulfillAsync()](/api/class-route.mdx#route-fulfill)。你可以通過選項覆蓋回應的個別欄位:
...
Playwright 支援 [WebSockets](https://developer.mozilla.org/en-US/docs/Web/API/WebSockets_API) 檢查。每次建立 WebSocket 時，會觸發 [Page.WebSocket](/api/class-page.mdx#page-event-web-socket) 事件。此事件包含 [WebSocket] 實例以進一步檢查 web socket 幀:
