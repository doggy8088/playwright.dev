...
### 支援Xunit
* 透過 [Microsoft.Playwright.Xunit](https://www.nuget.org/packages/Microsoft.Playwright.Xunit) 支援xUnit 2.8+。請參考我們的[開始使用](./intro.mdx)指南以了解更多。
...
### 重大變更
* [Expect(Locator).ToBeEditableAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-be-editable) 和 [Locator.IsEditableAsync()](/api/class-locator.mdx#locator-is-editable) 現在在目標元素不是 `<input>`、`<select>` 或其他許多可編輯元素時會拋出例外。
...
### 其他
- 將不再提供WebKit 在 Ubuntu 20.04 和 Debian 11 上的更新。我們建議將您的作業系統更新到較新版本。
...
- 新方法 [BrowserContext.NewCDPSessionAsync()](/api/class-browsercontext.mdx#browser-context-new-cdp-session) 和 [Browser.NewBrowserCDPSessionAsync()](/api/class-browser.mdx#browser-new-browser-cdp-session) 分別為頁面和瀏覽器建立 [Chrome DevTools Protocol](https://chromedevtools.github.io/devtools-protocol/) 工作階段。
...
- [Locator.SelectOptionAsync()](/api/class-locator.mdx#locator-select-選項) 現在按值或標籤進行比對：
...
- [Page.GetByLabel()](/api/class-page.mdx#page-get-by-label) 透過關聯標籤的文字定位表單控制項。
- [Page.GetByTestId()](/api/class-page.mdx#page-get-by-test-id) 以根據元素的 `data-testid` 屬性定位元素（可配置其他屬性）。
- [Page.GetByPlaceholder()](/api/class-page.mdx#page-get-by-placeholder) 根據預留位置文字定位輸入元素。
- [Page.GetByAltText()](/api/class-page.mdx#page-get-by-alt-text) 根據替代文字定位元素，通常是圖像。
...
- Codegen 現在支援MSTest and NUnit frameworks.
- 現在支援ASP .NET。

### 行為變更

許多 Playwright API 已經支援 `WaitUntil: WaitUntilState.DOMContentLoaded` 選項。例如：
...
等待定位器解析為具有指定狀態的單一元素。預設為 `state: 'visible'`。
...
在 [Locator.WaitForAsync()](/api/class-locator.mdx#locator-wait-for) 閱讀更多。

### 🎭 Playwright Trace Viewer
- 使用 `pwsh bin/Debug/netX/playwright.ps1 show-trace` 執行追蹤檢視器，並將追蹤檔案拖曳至追蹤檢視器 PWA
- 改善動作目標的視覺標示

在[追蹤檢視器](./trace-viewer)閱讀更多。
...
- **🔎 增強的 HAR** 具有請求和回應的主體大小。透過 [Browser.NewContextAsync()](/api/class-browser.mdx#browser-new-context) 中的 `recordHar` 選項。使用
...
- 隨附 [codegen](./codegen.mdx) 和 [trace viewer](./trace-viewer.mdx) 工具
