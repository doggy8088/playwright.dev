...
### 雜項
- 在 [ApiRequest.NewContextAsync()](/api/class-apirequest.mdx#api-request-new-context) 中新增選項 [MaxRedirects](/api/class-apirequest.mdx#api-request-new-context-option-max-redirects) 來控制最大重新導向次數。
...
### 亮點
* [BrowserContext.StorageStateAsync()](/api/class-browsercontext.mdx#browser-context-storage-state) 的新選項 [IndexedDB](/api/class-browsercontext.mdx#browser-context-storage-state-option-indexed-db) 允許儲存和還原 IndexedDB 內容。當您的應用程式使用 [IndexedDB API](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API) 儲存身份驗證權杖（如 Firebase Authentication）時很有用。
...
* 新選項 [FailOnStatusCode](/api/class-apirequest.mdx#api-request-new-context-option-fail-on-status-code) 使通過 [APIRequestContext] 發出的所有 fetch 請求在回應碼不是 2xx 和 3xx 時拋出例外。
...
- 新屬性 `httpCredentials.send` in [ApiRequest.NewContextAsync()](/api/class-apirequest.mdx#api-request-new-context) that allows to either always send the `Authorization` header or only send it in response to `401 Unauthorized`.
...
- 新方法 [FormData.Append()](/api/class-formdata.mdx#form-data-append) allows to specify repeating fields with the same name in [`Multipart`](./api/class-apirequestcontext#api-request-context-fetch-option-multipart) option in `APIRequestContext.FetchAsync()`:
...
### 新的 API
- 新方法 [Page.UnrouteAllAsync()](/api/class-page.mdx#page-unroute-all) removes all routes registered by [Page.RouteAsync()](/api/class-page.mdx#page-route) and [Page.RouteFromHARAsync()](/api/class-page.mdx#page-route-from-har). Optionally allows to wait for ongoing routes to finish, or ignore any errors from them.
...
- 新選項 [HasNotText|HasNotTextRegex](/api/class-locator.mdx#locator-filter-option-has-not-text) in [Locator.Filter()](/api/class-locator.mdx#locator-filter)
- [Expect(Locator).ToBeAttachedAsync()](/api/class-locatorassertions.mdx#locator-assertions-to-be-attached)
- 新選項 [Timeout](/api/class-route.mdx#route-fetch-option-timeout) in [Route.FetchAsync()](/api/class-route.mdx#route-fetch)
...
- 新選項 [Name](/api/class-tracing.mdx#tracing-start-chunk-option-name) in method [Tracing.StartChunkAsync()](/api/class-tracing.mdx#tracing-start-chunk).
...
- 新選項 [MaxRedirects](/api/class-route.mdx#route-fetch-option-max-redirects) for method [Route.FetchAsync()](/api/class-route.mdx#route-fetch).
...
### 新的 API
- 新方法 [Route.FetchAsync()](/api/class-route.mdx#route-fetch) and new option `Json` for [Route.FulfillAsync()](/api/class-route.mdx#route-fulfill):
